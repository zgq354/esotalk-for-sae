...
class AttachmentController extends ETController {
	...
	public function action_index($attachmentId = false)
	{
		...
		$model = ET::getInstance("attachmentModel");
		//redirect to the stroge link
		$path = $model->path().$attachmentId.'_';
		//in SAE
		$s = new SaeStorage();
        if($s->fileExists(SAESTOR_DOMAIN,$path.$attachment["filename"])){
            $url = $s->getUrl(SAESTOR_DOMAIN,$path.urlencode($attachment["filename"]));
            redirect($url);
        }else{
            $this->render404(T("message.attachmentNotFound"), true);
			return false;
	...
	public function action_upload()
	{
		...
		$model = ET::getInstance("attachmentModel");
		$result = $uploader->handleUpload(PATH_SAESTOR.$model->path(), $attachmentId.'_'.$name);
